...
def process_image(method):
    ...
    if method == 'brightness_correction':
        ...
        show_image(corrected_img, 350, 120, 'Perbaikan Metode 1 (Kecerahan)')
    elif method == 'grayscale':
        corrected_img = grayscale(original_img)
        show_image(corrected_img, 650, 120, 'Perbaikan Metode 2 (Grayscale)')

# fungsi untuk menampilkan informasi pembuat program
def show_creator():
    creator_label = tk.Label(root, text='Nama : Mustafa Fauzy Tompoh    NIM : F55121003    Kelas : A')
